  ...
  getLatestBlog: asyncHandler(async (req, res) => {
    ...
    try {
      const blogs = await Blog.find({ draft: false })
        ...
        .limit(limit)
        .select(
          "isSensitive title des tags banner content activity blog_id publishedAt -_id"
        );
      res.json(blogs);
    } catch (error) {
      throw new Error(error);
  ...
  getTrendingBlogs: asyncHandler(async (req, res) => {
    ...
    try {
      const trendingBlogs = await Blog.find({ draft: false })
        ...
        .limit(limit)
        .select(
          "isSensitive title des tags banner content activity publishedAt blog_id -_id"
        );
  ...
  readBlog: asyncHandler(async (req, res) => {
    ...
    try {
      const blog = await Blog.findOneAndUpdate(
        ...
        .select(
          "isSensitive title des content banner activity publishedAt blog_id tags author"
        );
  ...
  searchBlogs: asyncHandler(async (req, res) => {
    ...
    try {
      const blogs = await Blog.find(findQuery)
        ...
        .sort({ publishedAt: -1 })
        .select(
          "isSensitive blog_id title des banner tags activity publishedAt -_id"
        )
  ...
  getBlogById: asyncHandler(async (req, res) => {
    ...
    try {
      const blog = await Blog.findOne({ blog_id }).select(
        "title banner des content tags isSensitive"
      );
      res.json(blog);
    } catch (error) {
      throw new Error(error);
